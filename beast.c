...
ZEND_BEGIN_ARG_INFO_EX(arginfo_beast_encode_file, 0, 0, 2)
    ZEND_ARG_TYPE_INFO(0, input_file, IS_STRING, 0)
    ZEND_ARG_TYPE_INFO(0, output_file, IS_STRING, 0)
    ZEND_ARG_TYPE_INFO(0, expire, IS_LONG, 0)
    ZEND_ARG_TYPE_INFO(0, type, IS_LONG, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_beast_avail_cache, 0, 0, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_beast_support_filesize, 0, 0, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_beast_file_expire, 0, 0, 0)
ZEND_END_ARG_INFO()

ZEND_BEGIN_ARG_INFO_EX(arginfo_beast_clean_cache, 0, 0, 0)
ZEND_END_ARG_INFO()
...
int filter_code_comments(char *filename, zval *retval)
{
    ...
    zend_string *content = zend_string_alloc(filesize, 0);
    fread(ZSTR_VAL(content), 1, filesize, fp);
    fclose(fp);
    ZSTR_VAL(content)[filesize] = '\0';

    ZVAL_STR(retval, content);
    return 0;
}
    ...
    int encrypt_type)
{
    ...
    /* Open output file */
    output_stream = php_stream_open_wrapper((char *)outputfile, "w+", REPORT_ERRORS, NULL);
...
cgi_compile_file(zend_file_handle *h, int type)
{
...
#else
    fp = fopen(ZSTR_VAL(h->filename), "rb");
#endif
    if (fp != NULL) {
        fd = fileno(fp);
    } else {
    ...
    retval = decrypt_file(ZSTR_VAL(h->filename), fd, &buf, &size,
                          &free_buffer, &ops);
    if (retval == -2) {
        php_error_docref(NULL, E_ERROR,
    ...
    if (beast_debug_mode && beast_debug_path) {
        ...
        if (access(beast_debug_path, F_OK) == 0) {
            ...
            sprintf(realpath, "%s/%s", beast_debug_path, ZSTR_VAL(h->filename));
...
PHP_MINIT_FUNCTION(beast)
{
    ...
    REGISTER_LONG_CONSTANT("BEAST_ENCRYPT_TYPE_XXTEA",
        BEAST_ENCRYPT_TYPE_XXTEA, CONST_CS|CONST_PERSISTENT);
...
PHP_FUNCTION(beast_encode_file)
{
    ...
    beast_write_log(beast_log_debug,
    "begin to beast_encode_file");
